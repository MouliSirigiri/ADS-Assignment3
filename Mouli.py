...
def map_corr(df, size=6):
    ...
    im = ax.matshow(corr, cmap='cubehelix_r')
...
def plot_normalized_data(df_normalized):
    """
    Plots a stacked bar chart for each column in a normalized dataframe.
    Parameters:
    df_normalized (pandas.DataFrame): Normalized dataframe.
    """
    ...
    ax.set_title('Stacked Bar Chart of Normalized Data')
    ax.set_xlabel('Year')
    ax.set_ylabel('Value')
...
def predict_future(Energy_use_data, countries, indicators, start_year, end_year):
...
if __name__ == '__main__':
    ...
    countries = ['India', 'United States', 'European Union', 'Canada']
...
predict_future(r"C:\Users\mouli\Downloads\worldbank.csv", [
               'India', 'European Union', 'United States', 'Canada'], ['Energy use (kg of oil equivalent per capita)'], 1980, 2014)
map_corr(df, size=8)
